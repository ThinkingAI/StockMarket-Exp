...
eda.desc_data(data)
eda.plot_data(data.volume,data.close)

ax1 = plt.subplot(311)
t1 = np.arange(630)
#eda.plot_data(t1,data.open[::-1])
...
# 默认5折交叉验证，只是这里的评价指标不再是精度，而是另一个函数R2，大体上，这个得分多少代表有多大百分比的回归结果可以被训练器覆盖和解释
from sklearn.model_selection import KFold,cross_val_score
def train_and_evaluate(clf, X_train, y_train):
    cv = KFold( n_splits = 5, shuffle=True, random_state=33)
    scores = cross_val_score(clf, X_train, y_train, cv=cv)
    print (clf.__class__,'--Average coefficient of determination using 5-fold cross validation:', np.mean(scores),scores)
